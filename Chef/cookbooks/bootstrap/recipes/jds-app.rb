...
template '/opt/bootstrap.sh' do
  ...
  mode '750'
  variables(
    machine_hostname: node['infra']['jds_app_machine_hostname'],
    facter_machine_type: node['infra']['jds_app_machine_type'],
    facter_env_name: node['infra']['facter_env_name'],
    facter_machine_name: node['infra']['svc_jds_app_srv_name'],
